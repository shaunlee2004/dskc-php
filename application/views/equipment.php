
	<div class="paddedImage" style="float: left; position: relative;">
	<?php
	echo HTML::image ( "/media/img/ISKF_patch.jpg" )?>
	<div class="caption">ISKF patch</div>
	</div>
	<div class="paddedImage" style="float: left; position: relative;">
	<?php
	echo HTML::image ( "/media/img/white_gloves.jpg", array (
			'height' => 182,
			'width' => 200 
	) )?>
	<div class="caption">White vinyl sparring gloves</div>
	</div>
	<div class="paddedImage" style="float: left; position: relative;">
	<?php
	echo HTML::image ( "/media/img/tokaido_karate_mouthguard.jpg", array (
			'height' => 200,
			'width' => 223 
	) )?>
	<div class="caption">Mouthguard</div>
	</div>
<br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/><br/>
	<div class="paddedImage" style="float: left; position: relative;">
		<?php
		echo HTML::image ( "/media/img/belts.jpg", array (
				'height' => 180,
				'width' => 354 
		) )?>
	<div class="caption">belts</div>

	</div>
	<div class="paddedImage" style="float: left; position: relative;">
		<?php
		echo HTML::image ( "/media/img/karate_gi_youth.jpg", array (
				'height' => 160,
				'width' => 213 
		) )?>
	<div class="caption">Youth/Beginner Karate Uniform (gi)</div>

	</div>

	<div class="paddedImage" style="float: left; position: relative;">
		<?php
		echo HTML::image ( "/media/img/karate_gi_adult.jpg", array (
				'height' => 200,
				'width' => 200 
		) )?>
	<div class="caption">Adult/Advanced Karate Uniform (gi)</div>

	</div>
